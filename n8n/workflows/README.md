...
| 파일명 | 상태 | 설명 |
| --- | --- | --- |
| `001_문의_등록.json` | 구현됨 | 등록 webhook, 입력 검증, Notion 페이지 생성, SMTP 관리자 메일, 성공 응답 반환 |
| `002_문의_완료.json` | 구현됨 | 완료 webhook, 입력 검증, Notion 상태/처리결과 업데이트, SMTP 메일 발송, 성공 응답 반환 |
...
- 메일 노드는 `SMTP` credential을 사용하고, 메일 실패 시에도 Notion 작업이 성공했다면 HTTP 200과 메일 상태 필드를 반환한다.
- 파일명은 이 문서의 목록과 맞춘다.
